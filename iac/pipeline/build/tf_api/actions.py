...
def list_all(item,org_name,payload,**kwargs):
    log = base_log.Log(name=f"List{item.title()}")
    tapi = connector.TfeWorkspace(payload)
    return_id_list = kwargs.get("return_id_list",False)
    get_items = kwargs.get("get_items",{})
    return_all = kwargs.get("return_all",False)
    print_response = kwargs.get("print_response",kwargs.get("print_it",False))
    ...
    while keep_calling:
        try:
            rsp = tapi.connect(
                api_call={
                    "endpoint":f"organizations/{org_name}/{item}?page%5Bnumber%5D={call_counter}&page%5Bsize%5D=20",
                    "type":"get"
                }
            )
            if print_response:
            ...
            log.error(f"Failed List {item.title()} | Error: {traceback.format_exc()}")
            error_counter += 1
            if error_counter > 3:
                keep_calling = False 
                break 
