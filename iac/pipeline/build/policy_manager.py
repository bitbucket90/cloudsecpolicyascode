...
class tfe_policies(object):
    ...
    def policy_set(self,pset,prj_to_ws_mapping):#org_id,org_name,ws_id_list
        ...
        # Push Empty - Create Policy Set but Don't apply to any workspace 
        push_empty = pset.get("push_empty",False)
        ...
        if len(ws_id_list):
            ...
            if self.payload["tfe"]["org_details"][org_id]["workspace"] not in exclude_ws:
                exclude_ws.append(self.payload["tfe"]["org_details"][org_id]["workspace"])#("ws-CoLUfqDa3bPCDR8a")
        ...
        if cont_processing:
            policy_set_config = {
                ...
                "workspace_ids": [] if push_empty == True else pset["workspace_ids"],
            ...
            for pkx,pkey in enumerate(pset["policies"]):
                ...
                if is_active:
                    pol_dict = {
                        ...
                        "enforcement_mode": "advisory" if push_empty == True else pkey.get("enforcement_mode","advisory"),
                        # "policy_path": "s3_bucket_encryption.rego",
                        "policy_code":pkey["policy_code"],
                        #"organization":pset["organization"]
                    }
                    set_policy_list[pkey["policy_name"]] = pol_dict #pkey["pid"].strip().lower()
            ...
            if len(set_policy_list):
                ...
                policy_set_config["policy_keys"] = list(set_policy_list)
                #set_policy_list = policy_set_config
                return policy_set_config,pset["psid"].strip().lower(),cont_processing
    ...
    def deployer(self,organization,policy_list,policy_holder,**kwargs):
        ...
        for psx,pset in enumerate(policy_list):
            ...
            if len(policies):
                ...
                policy_set_config,pset_uid,cont_processing = self.policy_set(pset,prj_to_ws_mapping)
                if cont_processing:
                    # Policy Set Passed Validation Adding to Queue
                    #for psc in policy_set_config: 
                    pset_uid = f"{pset_uid}_{policy_set_config['policy_engine']}" if pset_uid.lower() in policy_holder.keys() else pset_uid.lower()
                    policy_holder[pset_uid] = policy_set_config
                    # policy_holder.append(policy_set_config)#(psc)
                else:
            ...
            if len(org_list):
                for oix,org in enumerate(org_list):
                    ...
                    if len(policies_to_deploy):
                        ...
                        cont_deployment = self.payload.get("deploy_polices",True)
                        if cont_deployment:
    ...
    def tfe_set_vars(self,policies,**kwargs):
        ...
        tfe = dedent(f'''\
        policy_keys = {json.dumps(list(policies.keys()))}
...
def main_handler():
    ...
    payload = {
        ...
        "tfe":{
            ...
            "policy_workspaces":{
                "banzai_dev": "ws-Yz8otywWDUWupQwG",# tfe-policy-deployer-dev#ws-CoLUfqDa3bPCDR8a
                "banzai_qa": "",
                "banzai_prod":""
            }
        }
    }
    payload["deploy_polices"] = True 
