...
resource "tfe_policy" "custom_policy" {
  ...
  # Required Arguments
  #---------------------------------
  for_each               = toset( var.policy_keys )#{ for k, v in var.policies: k => v }
  name                   = var.policies[each.key].policy_name #each.value.policy_name
  policy                 = var.policies[each.key].policy_code#each.value.policy_code
  #---------------------------------
  # Required Arg for OPA Rules
  #---------------------------------
  query                  = var.policies[each.key].opa_query#each.value.opa_query
  #---------------------------------
  # Optional Arguments
  #---------------------------------
  description            = var.policies[each.key].policy_description#each.value.policy_description # Default: ""
  organization           = var.organization # Default: Must be in provider block
  kind                   = var.policy_engine # Default: Sentinel 
  enforce_mode           = var.policies[each.key].enforcement_mode #each.value.enforcement_mode # Default: Advisory
}
