...
class TfeWorkspace(object):
    ...
    def connect(self,api_call):
        ...
        verify_cert = self.payload["tfe"].get("verify",True)
    ...
    def error_check(self,resp,url):
        if resp.status_code == 401:
            raise Exception(f"ERROR: 401 Auth Failed - {resp.text} | {url}")
        elif 400 <= resp.status_code < 500:
            raise Exception(f"ERROR: 400-499 Query to Terraform Failed - {resp.text} | {url}")
        elif 500 <= resp.status_code < 600:
            raise Exception(f"ERROR: 500-599 Terraform Interanl Server Error - {resp.text} | {url}")
